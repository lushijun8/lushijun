<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Log_Error_List.aspx.cs" Inherits="Web.Manager.Log.Log_Error_List" %>
			            ...
			            <ItemTemplate>
				            <tr <%# (Container.ItemIndex+1)%2==0?"class='b'":"" %> onclick="javascript:showhide('v_<%# Eval("id")%>');" style="cursor:pointer;">
                                 <td><%# Eval("id")%></td>
			                        ...
			                        <td><%# Eval("ip")%></td>
			                        <td><%# Eval("server_ip")%></td>
			                        <td><%# Eval("title")%></td>
			                        <td><%# Eval("username")%></td>
			                        <td><a href="<%# Business.Config.HostUrl %>/Manager/Log/Log_Error_List.aspx?keyword=<%# Server.UrlEncode(Eval("pageurl").ToString())%>"><%# Eval("pageurl")%></a> 
                                    ...
                                    <td><%# Eval("IsMy").ToString()=="1"?"<a href=\""+Business.Config.HostUrl+"/Manager/WebSite/WebSite_My_PageUrl_Delete.aspx?PageUrl="+Com.Common.EncDec.Encrypt(Eval("pageurl").ToString(),this.Encrypt_key)+"&BackUrl="+Server.UrlEncode(Business.Config.HostUrl+"/Manager/Log/Log_Error_List.aspx?page="+this.P_page+"&keyword="+Server.UrlEncode(this.txtKeyword.Text))+"\" onclick=\"javascript:return confirm_me('"+Eval("pageurl").ToString()+"')\">删除</a>":"<a href=\""+Business.Config.HostUrl+"/Manager/WebSite/WebSite_My_PageUrl_Add.aspx?PageUrl="+Com.Common.EncDec.Encrypt(Eval("pageurl").ToString(),Encrypt_key)+"&BackUrl="+Server.UrlEncode(Business.Config.HostUrl+"/Manager/Log/Log_Error_List.aspx?page="+this.P_page+"&keyword="+Server.UrlEncode(this.txtKeyword.Text))+"\" onclick=\"javascript:return confirm_me('"+Eval("pageurl").ToString()+"')\">认领</a>"%></td> 
					                <td><%# Eval("teamname")%></td>
			                        <td>详情+</td>					           
				            </tr>
                             <tr <%# (Container.ItemIndex+1)%2==0?"class='b'":"" %> id="v_<%# Eval("id")%>" style="<%# Container.ItemIndex==0?"":"display:none;"%>background-color:lightyellow;">
                                 <td colspan="11" style="border:1px solid #cccccc">
