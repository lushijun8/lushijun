<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DataBase_My_Sql_Ignore_List.aspx.cs" Inherits="Web.Manager.Sql.DataBase_My_Sql_Ignore_List" %>
			...
			<!--开始-->
                ...
                <%-- <p>&nbsp;</p>
		                <asp:Repeater ID="rpt_Date" runat="server" EnableViewState="false">
			                <ItemTemplate>
				                 <a <%# Eval("isCurrentDate").ToString()=="1"?"class=button":"class=button_off"%> href="DataBase_My_Sql_Ignore_List.aspx?orderby=<%=Server.UrlEncode(this.P_orderby)%>&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%# Eval("Date")%>"><%# Eval("Date")%></a>
			                </ItemTemplate>
		                </asp:Repeater> --%>
	            <!--开始-->
	            <table id="messagelist" width="100%" border="0" cellspacing="0" cellpadding="0" >
		            <tr> 
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=last_execution_time+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>最后执行时间</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=DATABASE_IP+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>数据库IP</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=DATABASE_USER+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>数据库用户</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=DATABASE_NAME+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>数据库名称</th>
			            <th> 
                            <a href="javascript:void(0)" onclick="javascript:showhide_td(this);" id="showhide_td">执行SQL＋＋＋</a>  
			            </th> 
                        <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=SQL_ERROR+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>漏洞与问题</th>
                        <th>忽略人</th>
                        <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=WebManager_name+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>负责人</th>
                        <th>认领</th>
                        <th>删除</th>
			            <th>长度</th> 
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=execution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>次</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=total_worker_time%2fexecution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>占cpu(毫秒)</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=total_elapsed_time%2fexecution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>执行(毫秒)</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=total_physical_reads%2fexecution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>物理读(次)</th>
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=total_logical_reads%2fexecution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>逻辑读(次)</th> 
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=total_logical_writes%2fexecution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>逻辑写(次)</th> 
			            <th><a href="DataBase_My_Sql_Ignore_List.aspx?orderby=total_rows%2fexecution_count+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>" class="orderby">↓</a>影响(条)</th> 
			            <th>详情＋</th> 
		            </tr>
