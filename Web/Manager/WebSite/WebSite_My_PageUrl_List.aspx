<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WebSite_My_PageUrl_List.aspx.cs" Inherits="Web.Manager.WebSite.WebSite_My_PageUrl_List" %>
			...
			<!--开始-->
                ...
                <asp:Button CssClass="button" ID="btnSearch" runat="server" Text="查找" OnClick="btnSearch_Click" />&nbsp;
                  <p>&nbsp;</p>
		                <asp:Repeater ID="rpt_Date" runat="server" EnableViewState="false">
			                <ItemTemplate>
				                 <a <%# Eval("isCurrentDate").ToString()=="1"?"class=button":"class=button_off"%> href="WebSite_My_PageUrl_List.aspx?orderby=<%=Server.UrlEncode(this.P_orderby)%>&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%# Eval("Date")%>"><%# Eval("Date")%></a>
			                </ItemTemplate>
		                </asp:Repeater> 
	            <table id="messagelist" width="100%" border="0" cellspacing="0" cellpadding="0" >
		            <tr>
			           ...
			           <th>请求页面</th>
			           <th>负责人</th>
			           <th><a href="WebSite_My_PageUrl_List.aspx?orderby=createtime&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>认领时间</th>  
			           <th><a href="WebSite_My_PageUrl_List.aspx?orderby=lastconnecttime&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>最后执行时间</th>    
                        <th>操作</th> 
			            <th><a href="WebSite_My_PageUrl_List.aspx?orderby=request_count&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>访次(今)</th>
			            <th><a href="WebSite_My_PageUrl_List.aspx?orderby=connect_times_min&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>连接数据库次数(今)<a href="WebSite_My_PageUrl_List.aspx?orderby=connect_times_max&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a> 平均<a href="WebSite_My_PageUrl_List.aspx?orderby=connect_times_avg&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a></th>
			            <th><a href="WebSite_My_PageUrl_List.aspx?orderby=duration_min&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>今日执行时间(毫秒)<a href="WebSite_My_PageUrl_List.aspx?orderby=duration_max" class="orderby&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a> 平均<a href="WebSite_My_PageUrl_List.aspx?orderby=duration_avg" class="orderby&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a></th>
		            </tr>
		            <asp:Repeater ID="rptLog" runat="server" EnableViewState="false">
			            <ItemTemplate>
				            <tr <%# (Container.ItemIndex+1)%2==0?"class='b'":"" %>>
                                ...
                                <td><%# "<a href=\""+Business.Config.HostUrl+"/Manager/WebSite/WebSite_My_PageUrl_Delete.aspx?PageUrl="+Com.Common.EncDec.Encrypt(Eval("pageurl").ToString(),this.Encrypt_key)+"&BackUrl="+Server.UrlEncode(Business.Config.HostUrl+"/Manager/WebSite/WebSite_My_PageUrl_List.aspx?keyword="+Server.UrlEncode(this.txtKeyword.Text)+"&today="+this.P_Today)+"\" onclick=\"javascript:return confirm_me('"+Eval("pageurl").ToString()+"')\">删除</a>"%></td> 
