<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Depend_List_My.aspx.cs" Inherits="Web.Manager.Depend.Depend_List_My" %>
        ...
        <td class="c2">
			...
			<!--开始-->
                ...
                <asp:Button CssClass="button" ID="btnSearch" runat="server" Text="查找" OnClick="btnSearch_Click" />&nbsp;
                 <p>&nbsp;</p>
		                <asp:Repeater ID="rpt_Date" runat="server" EnableViewState="false">
			                <ItemTemplate>
				                 <a <%# Eval("isCurrentDate").ToString()=="1"?"class=button":"class=button_off"%> href="Depend_List_My.aspx?orderby=<%=Server.UrlEncode(this.P_orderby)%>&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%# Eval("Date")%>"><%# Eval("Date")%></a>
			                </ItemTemplate>
		                </asp:Repeater> 
                 <!--开始-->                  
	            <table id="messagelist" width="100%" border="0" cellspacing="0" cellpadding="0" >
		            <tr>
			           <th>序号</th>
			           <th><a href="Depend_List_My.aspx?orderby=DEPEND_PAGEURL&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>被调用的接口URL</th>
                       <th><a href="Depend_List_My.aspx?orderby=WebManager_RealName+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>负责人</th> 
                       <th><a href="Depend_List_My.aspx?orderby=Depend_WebManager_RealName+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>调用人</th>
                       <th><a href="Depend_List_All.aspx?orderby=DEPEND_TIMEOUT+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>超时设置</th> 
			           <th><a href="Depend_List_My.aspx?orderby=DEPEND_COUNT+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>请求数</th>
                       <th><a href="Depend_List_My.aspx?orderby=DEPEND_COUNT_ERROR+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>错误数</th>
                       <th><a href="Depend_List_My.aspx?orderby=DEPEND_ERROR_RATE+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>错误率</th>
                       <th><a href="Depend_List_All.aspx?orderby=DEPEND_COUNT_TIMEOUT+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>超时数</th>
                       <th><a href="Depend_List_All.aspx?orderby=DEPEND_TIMEOUT_RATE+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>超时率</th>
			           <th><a href="Depend_List_My.aspx?orderby=DEPEND_TIMESPAN_MIN+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>耗时<a href="Depend_List_My.aspx?orderby=DEPEND_TIMESPAN_MAX+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a></th> 
			           <th><a href="Depend_List_My.aspx?orderby=DEPEND_TIMESPAN_AVERAGE+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>平均</th>  
                       <th><a href="Depend_List_My.aspx?orderby=DEPEND_CONTENTLENGTH+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>长度</th> 
                       <th><a href="Depend_List_My.aspx?orderby=DEPEND_CREATETIME+DESC&keyword=<%=Server.UrlEncode(this.txtKeyword.Text) %>&today=<%=this.P_Today%>" class="orderby">↓</a>最后调用时间</th> 
			           </tr>
			            ...
			            <ItemTemplate>
				            <tr <%# (Container.ItemIndex+1)%2==0?"class='b'":"" %>>
					           ...
					           <td<%# this.P_orderby.ToUpper()=="DEPEND_PAGEURL"?" class=orderby":""%>><img src="<%=Business.Config.HostUrl %>/images/<%# Eval("DEPEND_CONTENTTYPE_NEW")%>.gif" height="21"/> <a href="<%=Business.Config.HostUrl %>/Manager/Depend/Depend_View.aspx?v=<%# Com.Common.EncDec.Encrypt(Eval("DEPEND_PAGEURL").ToString()+","+Eval("STATS_DATE").ToString(),this.Encrypt_key) %>" target="_blank" titles="<%# Eval("DEPEND_PAGEURL_DETAIL")%>" class="tooltip"><%# Eval("DEPEND_PAGEURL")%></a>
                                      <%# Eval("Depend_PageUrl_Out").ToString()!=""?"<span class=\"depend_tooltip\" titles=\""+Eval("Depend_PageUrl_Out").ToString().Replace("\"","'")+"\"></span>":"" %></a>
					            ...
					            <td<%# this.P_orderby.ToUpper()=="DEPEND_WEBMANAGER_REALNAME DESC"?" class=orderby":""%>><%# Eval("Depend_WebManager_RealName")%></td> 
