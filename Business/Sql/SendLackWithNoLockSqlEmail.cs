...
{
    public class SendLackWithNoLockSqlEmail
    {
        ...
        { 
            ...
            if (System.DateTime.Now.Hour >= 9 && System.DateTime.Now.Hour <= 16)
            {
                ...
                string CacheName = "SendLackWithNoLockSqlEmail";
                ...
                {
                    System.Web.HttpRuntime.Cache.Add(CacheName, "SendLackWithNoLockSqlEmail", null, System.DateTime.Now.Add(new TimeSpan(0, 20, 0)), System.TimeSpan.Zero, System.Web.Caching.CacheItemPriority.Normal, null);
                    #region style
                    string style = @"* {
...
";
                    ...
                    //admin_webmanager.WEBMANAGER_NAME = "qiuxiaozhou";
                    ...
                    while (admin_webmanager.Next())
                    {
                        ...
                        string body = "<html xmlns=\"http://www.w3.org/1999/xhtml\" ><head><title>你的SQL缺少WITH(NOLOCK)~[系统邮件请勿回复]</title><style type=\"text/css\">" + style + "</style></head><body  class=\"Body_Right\"><div class=\"Body_Content\"><div class=\"Body_PageContent\">" +
                            "Hi!!&nbsp;&nbsp;" + admin_webmanager.WEBMANAGER_REALNAME + " 同学.<br/><br/>" +
                           "&nbsp;&nbsp;我们根据URL认领情况匹配出以下SQL疑似你负责的，并且缺少WITH(NOLOCK)，请立刻认领并针对发现的漏洞与问题进行优化，如果确定不是您负责的请点击“忽略”即可。<br/>" +
                           "<a href=\"" + Business.Config.HostUrl + "/Manager/Share/Share_Article_View.aspx?Article_Id=51F4FAE83D761567\">SELECT 时为何要加 WITH(NOLOCK)？</a><br/><br/><a href=\"" + Business.Config.HostUrl + "/Manager/Sql/DataBase_Sql_List_LikeMine.aspx\">查看更多疑似我的SQL</a><br/>";
                        #region 获取SQL
                        int counts = 3;
                        string sql = @"
                                    ...
                                    if(@counts<" + (counts * 3).ToString() + @")
                                        BEGIN
                                            ...
                                            AND Lack_With_NoLock_Count>0
                                            AND (SEEMLIKE_WEBMANAGER_NAME <> '' AND SEEMLIKE_WEBMANAGER_NAME IS NOT NULL)
											AND STATS_DATE > Dateadd(day,-2,CAST(@today AS DATETIME))    
                        ...
                        if (oDt_database_sql_stats.Rows.Count > 0)
                        {
                            ...
                            mail.MailTitle = "你的SQL缺少WITH(NOLOCK)~[系统邮件请勿回复," + System.DateTime.Now.ToString() + "]";
                            mail.MailContent = body;
                            #endregion
                            if (!mail.SendOneMailByHTML())
                            {
                                Business.MallLog.Create(0, "Web.Report", "TeamTool发送:你的SQL缺少WITH(NOLOCK)~邮件失败", "发送给" + admin_webmanager.WEBMANAGER_NAME + "失败", "127.0.0.1");
                            }
                            else
                            {
                                ...
                                foreach (DataRow oDr_database_sql_stats in oDt_database_sql_stats.Rows)
                                {
                                    ...
                                    {
                                        Business.MallLog.Create(0, "Web.Report", "TeamTool发送完你的SQL缺少WITH(NOLOCK)~邮件后database_sql_sendemail.Insert()失败", "发送给" + admin_webmanager.WEBMANAGER_NAME + "失败", "127.0.0.1");
                                    }
                                    k++;
                                    if (k >= i)
                                    {
