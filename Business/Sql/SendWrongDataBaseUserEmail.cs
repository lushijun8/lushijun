...
{
    public class SendWrongDataBaseUserEmail
    {
        public static void Run()
        {
            ...
            if (System.DateTime.Now.Hour >= 9 && System.DateTime.Now.Hour <= 16)
            {
                ...
                string CacheName = "SendWrongDataBaseUserEmail";
                ...
                {
                    System.Web.HttpRuntime.Cache.Add(CacheName, "SendWrongDataBaseUserEmail", null, System.DateTime.Now.Add(new TimeSpan(0, 60, 0)), System.TimeSpan.Zero, System.Web.Caching.CacheItemPriority.Normal, null);
                    #region style
                    string style = @"* {
...
";
                    ...
                    //admin_webmanager.WEBMANAGER_NAME = "huangmuxiang";
                    ...
                    while (admin_webmanager.Next())
                    {
                        ...
                        string body = "<html xmlns=\"http://www.w3.org/1999/xhtml\" ><head><title>你的只读SQL使用了可写数据库连接串~[系统邮件请勿回复]</title><style type=\"text/css\">" + style + "</style></head><body  class=\"Body_Right\"><div class=\"Body_Content\"><div class=\"Body_PageContent\">" +
                            "Hi!!&nbsp;&nbsp;" + admin_webmanager.WEBMANAGER_REALNAME + " 同学.<br/><br/>" +
                           "&nbsp;&nbsp;我们根据URL认领情况匹配出以下SQL疑似你负责的，并且只读SQL却使用了可写数据库用户，或者使用了可写数据库连接串，SQL读写权限未分离，有潜在危险，容易被攻击，请立刻认领并针对发现的漏洞与问题进行优化，如果确定不是您负责的请点击“忽略”即可。<br/>" +
                           "<a href=\"" + Business.Config.HostUrl + "/Manager/Share/Share_Article_View.aspx?Article_Id=4F50CCAE46B06885\">为什么数据库用户要读写分离？</a><br/>" +
                           "<a href=\"" + Business.Config.HostUrl + "/Manager/Share/Share_Article_View.aspx?Article_Id=44A60FFA340973D7\">网站开发安全规范（有SQL读写权限分离的解释）</a><br/><br/>" +
                           "<a href=\"" + Business.Config.HostUrl + "/Manager/Sql/DataBase_Sql_List_LikeMine.aspx\">查看更多疑似我的SQL</a><br/>";
                        #region 获取SQL
                        int counts = 10;
                        string sql = @"
                                    ...
                                    SELECT @counts=count(1) FROM [DataBase_Sql_SendEmail_WrongDataBaseUser] WITH (NOLOCK) WHERE 
                                    ...
                                    if(@counts<" + (counts * 3).ToString() + @")
                                        BEGIN
                                            ...
                                            AND (SQL_MD5+CONVERT(VARCHAR(100), STATS_DATE, 23) NOT IN (SELECT SQL_MD5+CONVERT(VARCHAR(100), STATS_DATE, 23) FROM [DataBase_Sql_SendEmail_WrongDataBaseUser] WITH (NOLOCK) WHERE WebManager_name = @WebManager_name )  )
                                            AND iswrongdatabaseuser=1 and isruntimesql=1
                        ...
                        if (oDt_database_sql_stats.Rows.Count > 0)
                        {
                            ...
                            //MailUserList = "lushijun@fang.com";
                            Com.Mail.MailUtil mail = new Com.Mail.MailUtil();
                            mail.MailUserList = MailUserList;
                            mail.CopyToMailUserList = "lushijun@fang.com";
                            mail.MailTitle = "你的只读SQL使用了可写数据库连接串~[系统邮件请勿回复," + System.DateTime.Now.ToString() + "]";
                            mail.MailContent = body;
                            #endregion
                            if (!mail.SendOneMailByHTML())
                            {
                                Business.MallLog.Create(0, "Web.Report", "TeamTool发送:你的只读SQL使用了可写数据库连接串~邮件失败", "发送给" + admin_webmanager.WEBMANAGER_NAME + "失败", "127.0.0.1");
                            }
                            else
                            {
                                #region  DATABASE_SQL_SENDEMAIL_WrongDataBaseUser.Insert()
                                int k = 0;
                                foreach (DataRow oDr_database_sql_stats in oDt_database_sql_stats.Rows)
                                {
                                    Entity.TEAMTOOL.DATABASE_SQL_SENDEMAIL_WRONGDATABASEUSER database_sql_sendemail_wrongdatabaseuser = new Entity.TEAMTOOL.DATABASE_SQL_SENDEMAIL_WRONGDATABASEUSER();
                                    database_sql_sendemail_wrongdatabaseuser.WEBMANAGER_NAME = admin_webmanager.WEBMANAGER_NAME;
                                    database_sql_sendemail_wrongdatabaseuser.SQL_MD5 = oDr_database_sql_stats[database_sql_sendemail_wrongdatabaseuser._SQL_MD5].ToString();
                                    database_sql_sendemail_wrongdatabaseuser.STATS_DATE = DateTime.Parse(oDr_database_sql_stats[database_sql_sendemail_wrongdatabaseuser._STATS_DATE].ToString());
                                    database_sql_sendemail_wrongdatabaseuser.CREATETIME = DateTime.Now;
                                    if (!database_sql_sendemail_wrongdatabaseuser.Insert())
                                    {
                                        Business.MallLog.Create(0, "Web.Report", "TeamTool发送完你的SQL执行较慢~邮件后database_sql_sendemail_wrongdatabaseuser.Insert()失败", "发送给" + admin_webmanager.WEBMANAGER_NAME + "失败", "127.0.0.1");
                                    }
                                    k++;
                                    if (k >= i)
                                    {
